        ...
        const CONFIG = {
          SWAP_CONTRACT: "0x37757AD0A16bFa13184507d16f34f043b7A63382", // Replace with deployed contract
        ...
        async function calculateOutput() {
            ...
            if (!amount || !walletAddress || parseFloat(amount) <= 0) {
                toAmount.value = '';
                document.getElementById('feeAmount').textContent = swapDirection === 'usdc' ? '0.00 EURC' : '0.00 USDC';
                return;
            }

            try {
                const amountWei = ethers.utils.parseUnits(amount, 6); // USDC/EURC have 6 decimals
                let quote, fee;
                ...
                toAmount.value = ethers.utils.formatUnits(quote, 6);
                document.getElementById('feeAmount').textContent = 
                    ethers.utils.formatUnits(fee, 6) + (swapDirection === 'usdc' ? ' EURC' : ' USDC');

                // Update button state
                if (parseFloat(ethers.utils.formatUnits(quote, 6)) >= 2.0) {
                    swapBtn.disabled = false;
        ...
        swapBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const tokenContract = swapDirection === 'usdc' ? usdcContract : eurcContract;
                const allowance = await tokenContract.allowance(walletAddress, CONFIG.SWAP_CONTRACT);

                if (allowance.lt(amountWei)) {
                    showMessage('Approving tokens...', 'success');
                    const approveTx = await tokenContract.approve(CONFIG.SWAP_CONTRACT, ethers.constants.MaxUint256);
        ...
        async function updateBalances() {
            ...
            try {
                ...
                document.getElementById('fromBalance').textContent = 
                    ethers.utils.formatUnits(swapDirection === 'usdc' ? usdcBalance : eurcBalance, 6);
                
                document.getElementById('toBalance').textContent = 
                    ethers.utils.formatUnits(swapDirection === 'usdc' ? eurcBalance : usdcBalance, 6);
            } catch (error) {
        ...
        async function updatePoolStats() {
            try {
                ...
                const usdcFormatted = parseFloat(ethers.utils.formatUnits(usdcPool, 6));
                const eurcFormatted = parseFloat(ethers.utils.formatUnits(eurcPool, 6));
                const total = usdcFormatted + eurcFormatted;

                document.getElementById('usdcPool').textContent = 
                    usdcFormatted.toLocaleString('en-US', { maximumFractionDigits: 2 });
                
                document.getElementById('eurcPool').textContent = 
                    eurcFormatted.toLocaleString('en-US', { maximumFractionDigits: 2 });
